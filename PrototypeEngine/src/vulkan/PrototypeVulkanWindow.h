...
struct PrototypeVulkanRenderer;
struct VkInstanceCreateInfo;

struct PrototypeVulkanWindow final : PrototypeWindow
{
    PrototypeVulkanWindow();

    ~PrototypeVulkanWindow() final = default;
  ...
  private:
    friend struct PrototypeVulkanRenderer;

    void createSurface(VkInstance instance, VkSurfaceKHR* surface);
    void getFramebufferExtent(i32* width, i32* height);
    void refreshTitleBar();
    void setPhysicalDeviceStr(std::string value);
