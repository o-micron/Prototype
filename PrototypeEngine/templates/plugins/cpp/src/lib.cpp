...
PluginLoadProtocol(PrototypeEngineContext* engineContext, PrototypeLoggerData* loggerData)
{
    ...
    consoleLog(nullptr, __FILE__, __LINE__, "{{ PrototypePluginProjectName }} is now loaded.");
    return true;
}
...
PluginReloadProtocol(PrototypeEngineContext* engineContext, PrototypeLoggerData* loggerData)
{
    ...
    consoleLog(nullptr, __FILE__, __LINE__, "{{ PrototypePluginProjectName }} is now reloaded.");
    return true;
}
...
PluginStartProtocol(void* object)
{
    ...
    snprintf(formatBuffer, 256, "{{ PrototypePluginProjectName }} is now attached to %s", objectName);
...
PluginEndProtocol(void* object)
{
    ...
    ObjectGetName(object, &objectName);
    char formatBuffer[256];
    snprintf(formatBuffer, 256, "{{ PrototypePluginProjectName }} is now dettached from %s", objectName);
    consoleLog(object, __FILE__, __LINE__, formatBuffer);
    return true;
}
...
{
    consoleLog(nullptr, __FILE__, __LINE__, "{{ PrototypePluginProjectName }} is now unloaded.");
    return true;
}
